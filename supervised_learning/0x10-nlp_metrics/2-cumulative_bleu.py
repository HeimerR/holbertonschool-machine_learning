...
def cumulative_bleu(references, sentence, n):
    ...
    precisions = []
    for i in range(n):
        precisions.append(calc_precision(references, sentence, i+1))
    ...
    w = np.empty((n,))
    w[:] = 1/n
    Bleu = BP * np.exp(np.sum(w*np.log(precisions)))
    print("Bleu: ", Bleu)
    score = sentence_bleu(references, sentence, weights=(0.25, 0.25, 0.25, 0.25))
